...
class ExchangeController(BaseController):
    ...
    def handle_get(self: BaseHTTPRequestHandler):
        data = Validator.is_exchange_fields(self.path) # {'from': 'RUB', 'to': 'USD', 'amount': '1000'}
        if not data:
        ...
        try:
            base_currency_model, target_currency_model = (CurrencyModel(code=data['from']),
                                                          CurrencyModel(code=data[
                                                              'to']))  # {'id': None, 'name': None, 'code': 'RUB', 'sign': None}
            ...
            # Передаем сервису модель
            response = CurrenciesService(exchange_rates_model).exchange_currency()
